...
- **Creative Commons / permissively licensed** — CC0 or CC-BY, and (this being a non-commercial project) NonCommercial/ShareAlike too: `CC-BY-NC`, `CC-BY-SA`, `CC-BY-NC-SA` (DECISIONS 2026-06-14). Avoid only NoDerivatives, since we retexture/retrim. Attribution recorded where the license asks.
...
Every file in `public/assets/` is generated in-repo or under a Creative Commons / permissive license — CC0, CC-BY, or (non-commercial project) NonCommercial/ShareAlike, just not NoDerivatives (DECISIONS 2026-06-14). Never Valve's or Blizzard's shipped files: not models, not textures, not icons, not the Reaver or Radiance fonts, not sounds. Heroes resemble their archetype through our assets (§1.1, `SPEC.md §9`). `ASSETS.md` records the source and license of every file, and a test can scan it so an unlicensed asset cannot slip in. The existing "no asset imports in source" guard (`src/test/data-lint.test.ts`) stays; assets load at runtime from `public/`, not through `import`.
