...
## 0.5 STATUS — what has shipped (S0–S5 done; S6 channel-DPS + swap-grace done; S7 real off-field summon bodies done)
...
- **S4 — items + readability.** The new tag items (`relay-standard`, `heralds-token`, `vanguard-sigil`, `echo-conduit`, `chainweaver-band`); reaction-preview on the swap prompt; the chain counter and off-field markers in the combat readout. The AI gambit reads `tag-in-ready` and `combo-setup-active` (`core/controllers.ts`) also landed. **Item tag-in lines are now real** (`ItemTagBoon` on `ItemDef`, resolved in `fireTagBoon` via `itemTagEffects`): the §7 legacy items carry their proposed lines (Mekansm heals, Guardian Greaves cleanses, Glimmer Cape/Pipe shield, Vladmir's lifesteal, Force Staff shoves, carry weapons grant a small selfish crumb), and the two stat-only items inject behavior — Vanguard Sigil grants a Bulwark tag-in to heroes without one, Echo Conduit leaves a lingering element field on a Soak tag. The §3.3 **aim cursor** (`TagBoonDef.aim`, threaded through `trySwap`/`fireTagBoon`, with a `tag-swap` targeting state in `input.ts`; Invoker's sunstrike uses it), the §9 **next-link hint** (`combatReadout().nextLink`, rendered in the readout), and the §9 **dull arrival beat** (`swap-flat` event → fainter audio/VFX when you swap with the gauge down) all shipped.
...
- **S7 — real off-field summon bodies.** The off-field summoners' tag-out legacy is now a real `summon` spawn (eidolons, spiderlings, familiars, the Spirit Bear, wolves, hawk+boar, zombies, treants, exorcism spirits, a tempest double, reflections/images, plague wards) instead of an S5 lingering-zone stand-in. Specs live in `SUMMON_BODIES` (`data/tag-boons.ts`); the bodies fight autonomously around the benched hero and persist on their own lifetime. Budget-neutral (a `summon` node scores like the `zone` it replaced), so the §4 data-lint stays green. Also a small cleanup: the vestigial `swapCooldownSec` / `resonanceSwapCooldownSec` tuning keys (left lowered to match the new floor) are removed; the swap path reads `swapFloorSec` / `resonanceSwapFloorSec` only.
- **S8 — the charge-meter toggle (§2.3).** The last back-pocket proposal, shipped as the opt-in it was always meant to be: `settings.swapCharges` (off by default, persisted + save-migrated, with a Settings checkbox) swaps the floor for a 2-charge meter — `trySwap` accrues fractional charges from sim time and spends one per swap, so you can swap twice fast and then wait `swapChargeRefillSec` (`resonanceSwapChargeRefillSec` under Resonance, both shaved by `swapCdReductionPct`). The readout shows charge pips (`combatReadout().swapCharges`). Headless + e2e cover the double-swap, the refill, the floor contrast when off, and the save round-trip. With this the whole document is implemented — nothing deferred.
...
3. **The last of §8 depth.** Off-field *channel* DPS (§8.2) and the swap-cancel grace window (§8.3) **shipped in S6**. The **charge-meter alternative (§2.3)** — deliberately left in the back pocket as a default — is now **shipped as the opt-in toggle the section describes**: `settings.swapCharges` (off by default, persisted + save-migrated) replaces the swap floor with a 2-charge meter (`TUNING.swapChargeMax` / `swapChargeRefillSec` / `resonanceSwapChargeRefillSec`, the refill shaved by `swapCdReductionPct`), gated in `trySwap`, surfaced as `combatReadout().swapCharges` (charge pips in the readout) and a Settings checkbox. The gauge model stays the default; the meter is the high-skill alternative. **Nothing in this doc is deferred any longer.**

5. ~~S4 item design was thinner than the spec (legacy items had no tag-in lines; Vanguard Sigil/Echo Conduit were stat-only), UX polish was missing the aim cursor / next-link hint / dull beat, and the browser tests had not caught up to the overworld tag verbs.~~ **Closed** — see the S4 line above. The under-tested acceptance corners (Resonance-off remove-on-swap, bench-time gauge rearm, Witch Doctor's off-field Death Ward, item tag lines, aim, next-link, dull beat) now have headless coverage in `gameplay-overhaul.test.ts`, and the overworld tag loop (gauge → boon → chain → off-field → reaction-preview, plus the HUD rendering) has live browser coverage in `e2e/overworld-tag.spec.ts`.

4. ~~Real `summon` bodies for off-field summoners (S5 used lingering zones).~~ **Closed in S7** — the off-field summoners (Enigma, Broodmother, Visage, Lone Druid, Lycan, Beastmaster, Undying, Nature's Prophet, Death Prophet, Arc Warden, Terrorblade, Naga Siren, Phantom Lancer, Venomancer) now leave **real `summon` bodies** on tag-out (and Venomancer's ward on tag-in), authored as modest swap-legacy `SummonSpec`s in `data/tag-boons.ts` and resolved through the existing `summon` `EffectNode` (`spawnSummon`). A summon is an independent unit with its own lifetime, so the body keeps fighting around the benched hero through the off-field window. The `summon` node scores the same as the `zone` it replaced, so the §4 budget and the inverse-power law are untouched. The genuinely field-flavored Drop heroes (Disruptor's static field, Dazzle's heal field, Techies' mines, the DoT-field nukers) stay as lingering zones, and the elemental enablers that need a ground aura for reactions (Nature's Prophet's dendro sprout) keep their field *alongside* the new bodies.
...
Considered: replace the floor with a **2-charge** swap meter (swap twice fast, then wait for a charge to refill). It feels great for burst rotations and is very Genshin-adjacent. Rejected as the default because it complicates the dodge-swap (you don't want to be out of charges when you need to peel) and the gauge model already delivers the "time your payoff" depth without rationing the *movement*. **Shipped as the high-skill `settings.swapCharges` toggle** (off by default): when on, `trySwap` accrues and spends from a 2-charge meter instead of checking the floor, the refill obeys `swapCdReductionPct` and the Resonance variant, and the readout shows charge pips. The gauge model remains the default.
...
**S5 — Author the full §6 table + first-class Soak.** Replace the role-templates with each hero's *signature* tag from §6 (§0.5 gap 1), keyed by hero id, so every hero is the combo piece the table describes. Promote **Soak** to a real archetype (§0.5 gap 2): a Soak tag lays a lingering **element field** — a `zone` whose tick re-applies the hero's element (and usually a slow/DoT) — so a benched soaker leaves the element on the ground for the next hero to detonate, on the overworld path, under Resonance. Off-field heroes (Enigma, Broodmother, Visage, Lone Druid, …) leave persistent off-field pressure on swap-out — originally a lingering field/legacy and, **as of S7, real `summon` bodies** (see §0.5) that fight on their own while the hero is benched. All authored inside the §4 budget bands so the data-lint stays green and the inverse-power law holds. Pure data plus the Soak helper — no new core surface.

**S6 — The remaining §8 depth.** *Shipped:* off-field *channel* DPS behind a per-ability `channel.offField: true` flag (§8.2) and the swap-cancel grace window (§8.3, reusing the dash's §3.0 timing contract — a swap during a cast point queues until the cast fires). The optional charge-meter toggle (§2.3) is now shipped too, as the opt-in `settings.swapCharges` rather than the default. Each piece is independent and additive.

**S7 — Real off-field summon bodies.** Replace the S5 lingering-zone stand-ins for the off-field summoners with real `summon` spawns (modest swap-legacy `SummonSpec`s in `SUMMON_BODIES`, resolved through the existing `summon` `EffectNode`). The bodies fight autonomously around the benched hero and persist on their own lifetime; budget-neutral against the §4 data-lint. Field-flavored Drop heroes and the dendro-soak enabler keep their zones. Headless test: a benched summoner's bodies spawn and keep damaging an enemy on their own. *Nothing deferred.*
